-- SWILL OS KERNEL v1.1 (с рабочим столом)
print("🔷 SWILL OS Kernel v1.1")
...
if WindowManager then
    ...
    -- Добавляем иконки приложений на рабочий стол
    local yPos = 20
    ...
    addIcon("Телепорт", Color3.fromRGB(0, 191, 255), function()
        if _G.SWILL_OS.modules.apps and _G.SWILL_OS.modules.apps.App_Teleport then
            _G.SWILL_OS.modules.apps.App_Teleport.run()
        else
            -- Если приложение не зарегистрировано, запускаем его напрямую
            local success, content = pcall(function()
...
print("🔷 SWILL OS загружена! Нажми F2 для скрытия/показа")
print("🔷 Нажми на иконку 'Телепорт' для открытия приложения")
