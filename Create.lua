...
local function sendChatMessage(msg)
 local chatRemote = replicatedStorage:FindFirstChild("DefaultChatSystemChatEvents") or replicatedStorage:FindFirstChild("SayMessageReq
 if chatRemote then
...
local function updateESP()
 ...
 hpBar.Position = UDim2.new(0, 0, 1, 0) hpBar.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
 ...
 espNames[plr].hpBar.BackgroundColor3 = hpPercent > 0.5 and Color3.fromRGB(0, 255, 0) or (hpPercent > 0.2 and Color3.f
 end
 end
...
local function fullShutdown()
 ...
 noclipEnabled = false updateNoclip()
 end
 if godModeEnabled then
...
TeleportTab:CreateButton({
 ...
 if not selectedPlayer.Character or not selectedPlayer.Character:FindFirstChild("HumanoidRootPart") then return Rayfield:Notify({T
...
TeleportTab:CreateButton({Name = "Телепорт к метке", Callback = function()
 ...
 else Rayfield:Notify({Title = "Ошибка", Content = "Нет сохранённой метки!", Duration = 2})
 end
end})
-- Вкладка ВИЗУАЛ
...
MoveTab:CreateSlider({Name = "Скорость бега", Range = {16, 300}, Increment = 1, Suffix = "WalkSpeed", CurrentValue = 16, Flag = "SpeedSli
MoveTab:CreateButton({Name = "Сбросить скорость", Callback = function() setSpeed(16) end})
MoveTab:CreateToggle({Name = "Бесконечный прыжок", CurrentValue = false, Flag = "InfiniteJump", Callback = function(value) infiniteJumpEn
