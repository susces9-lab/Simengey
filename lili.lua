...
-- Телепорт, ESP, полёт, метки, логи, анти-падение, noclip, бессмертие, отхил, невидимость
...
local function setXRay(value)
    if value then
        ...
        addLog("X-Ray включён (стены прозрачные)")
    else
        for _, part in pairs(workspace:GetDescendants()) do
            if part:IsA("BasePart") then
...
local function fullShutdown()
    ...
    -- Закрываем Rayfield окно
    Rayfield:Destroy()
    
    addLog("Скрипт полностью остановлен")
    print("SWILL ULTIMATE полностью остановлен")
end
...
VisualTab:CreateToggle({
    Name = "X-Ray (скрыть стены)",
    CurrentValue = false,
    Flag = "XRay",
    Callback = function(value) setXRay(value) end
})
...
MoveTab:CreateSection("Управление скриптом")
MoveTab:CreateButton({
    Name = "ПОЛНОСТЬЮ ЗАКРЫТЬ СКРИПТ",
    Callback = function()
        fullShutdown()
    end
})
...
HealTab:CreateButton({
    Name = "Вылечить 50 HP",
    Callback = function() healPlayer(50) end
})

HealTab:CreateButton({
    Name = "Вылечить 100 HP",
    Callback = function() healPlayer(100) end
})
...
-- Обновление noclip при изменении частей персонажа
